...
class DataMigrator:
    ...
    def migrate_personnel(self):
        ...
        for d in Departement.objects.all():
            dept_cache[d.nom.lower().strip()] = d
        ...
        def _dept_id(department_id_or_code):
            ...
            for d in Departement.objects.all():
                if code in d.nom.lower() or d.nom.lower().startswith(code):
                    return dept_cache.get(d.nom.lower().strip())
            return None
        ...
        for r in rows:
            ...
            if Personnel.objects.filter(nom=last_name, prenom=first_name).exists():
                continue
            ...
            count += 1
        self.success(f'{count} employés importés')
    ...
    def migrate_visites(self):
        ...
        vis_by_phone = {v.telephone: v.id for v in all_visiteurs if v.telephone}
        ...
        for r in rows:
            ...
            signature = self.get_col('visitor_in_out', r, 'visitor_signature')
            ...
            if arrival_date and arrival_time:
                visite_dt = datetime.combine(arrival_date, arrival_time)
            else:
                visite_dt = arrival_date
                if visite_dt:
                    visite_dt = datetime.combine(visite_dt, time(8, 0))

            depart_dt = None
            if release_date:
                dep_time = exit_time or time(17, 0)
                depart_dt = datetime.combine(release_date, dep_time)

            personnel = None
            if employee_id and employee_id > 0:
                try:
                    emp_idx = int(employee_id) - 1
                    all_p = list(Personnel.objects.all().only('id'))
                    if 0 <= emp_idx < len(all_p):
                        personnel = all_p[emp_idx]
                except (ValueError, IndexError):
                    pass

            type_v = type_default
            if visit_type_id:
                all_tv = list(TypeVisite.objects.all().only('id'))
                try:
            ...
            if not visiteur_id:
                alt_map = {str(v.id): v.id for v in all_visiteurs}
                user_alt_id_str = str(user_alt_id) if user_alt_id else ''
                if user_alt_id_str in alt_map:
                    visiteur_id = alt_map[user_alt_id_str]
            ...
            if len(batch) >= BATCH:
                ...
                batch = []
                self.stdout.write(f'    → {count} visites importées...', ending='\r')

        if batch:
            Visite.objects.bulk_create(batch, ignore_conflicts=True)

        self.success(f'{count} visites importées')
        return Visite.objects.count()

    def run_all(self):
        self.info('Début de la migration des données')
        self.info(f'Source : {OLD_DB_PATH}')
        self.info('')

        steps = [
            ('Départements', self.migrate_departements),
            ('Portes d\'entrée', self.migrate_portes_entree),
            ('Types de visite', self.migrate_types_visite),
            ('Créneaux semaine', self.migrate_creneaux),
        ...
        for label, method in steps:
            ...
            try:
                count = method()
                self.info(f'Total en base : {count}')
            except Exception as e:
                self.error(f'Échec {label} : {e}')
                import traceback
                traceback.print_exc()
                raise
            self.info('')
        ...
        self.write_sql_file()
        self.success('Migration terminée avec succès')
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(
            f'✓ {len(table_names)} tables trouvées : {", ".join(sorted(table_names))}'
        ))
        ...
        if skip_visites:
            original_migrate = migrator.migrate_visites
            migrator.migrate_visites = lambda: self.stdout.write(
                self.style.WARNING('  ⚠ Migration des visites ignorée (--skip-visites)')
            ) or 0
